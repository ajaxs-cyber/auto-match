      ...
      <section ref={sectionRef} id="templates" className="relative py-24 px-4 sm:px-6 lg:px-10" style={{ background: 'linear-gradient(to bottom, var(--canvas-base) 80%, #EDEAE4 100%)', zIndex: 1 }}>
        <div className="max-w-[1200px] mx-auto">
          <div className="text-center mb-8">
            <span className="text-xs font-semibold uppercase tracking-widest" style={{ color: 'var(--text-tertiary)' }}>{__('tpl.label')}</span>
            <h2 className="mt-3 text-[clamp(1.5rem,3vw,2.25rem)] font-bold" style={{ color: 'var(--text-primary)' }}>{__('tpl.title')}</h2>
            <p className="mt-2 text-base max-w-lg mx-auto" style={{ color: 'var(--text-secondary)' }}>{__('tpl.subtitle')}</p>
          </div>

          <div className="flex flex-col sm:flex-row items-center gap-3 mb-6 max-w-2xl mx-auto">
            <div className="flex-1 w-full relative">
              <Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2" style={{ color: 'var(--text-tertiary)' }} />
              <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder={__('tpl.searchPlaceholder')} className="w-full pl-9 pr-4 py-2.5 rounded-full text-sm border outline-none focus:ring-2" style={{ background: 'white', borderColor: 'var(--border-color)', color: 'var(--text-primary)', '--tw-ring-color': 'rgba(232,93,76,0.2)' } as React.CSSProperties} />
              ...
              <div key={tpl.id} className="template-card card-elevated overflow-hidden opacity-0 translate-y-12 transition-all duration-700 relative group" style={{ transitionTimingFunction: 'cubic-bezier(0.16, 1, 0.3, 1)' }} onMouseEnter={() => setHoveredId(tpl.id)} onMouseLeave={() => setHoveredId(null)}>
                <div className="aspect-[16/10] overflow-hidden relative">
                  ...
                  <div className={`absolute inset-0 bg-black/50 flex items-center justify-center gap-3 transition-opacity duration-300 ${hoveredId === tpl.id ? 'opacity-100' : 'opacity-0'}`}>
                    <button onClick={() => setPreviewTemplate(tpl)} className="w-10 h-10 rounded-full bg-white/20 backdrop-blur-sm flex items-center justify-center text-white border-none cursor-pointer hover:bg-white/30 transition-colors" title={__('tpl.preview')}><Eye size={18} /></button>
                    <button onClick={() => onUseTemplate(tpl.id)} className="px-5 py-2.5 rounded-full text-xs font-semibold uppercase tracking-wider cursor-pointer" style={{ background: 'var(--accent)', color: 'white' }}>{__('tpl.useTemplate')}</button>
                  </div>
                ...
                <div className="p-5">
                  ...
                  <button onClick={() => onUseTemplate(tpl.id)} className="w-full mt-4 py-2.5 rounded-full text-xs font-semibold uppercase tracking-wider cursor-pointer transition-all border hover:bg-[var(--text-primary)] hover:text-white" style={{ background: 'transparent', borderColor: 'rgba(26,43,60,0.15)', color: 'var(--text-primary)' }}>{__('tpl.useTemplate')}</button>
                </div>
              </div>
            ))}
          </div>
          {filtered.length === 0 && <div className="text-center py-16" style={{ color: 'var(--text-tertiary)' }}><Search size={40} className="mx-auto mb-3 opacity-30" /><p className="text-sm font-medium">{__('tpl.empty')}</p></div>}
        </div>
      </section>
          ...
          <div className="w-full max-w-[900px] max-h-[90vh] bg-white rounded-2xl shadow-2xl overflow-hidden flex flex-col" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-6 py-4 border-b" style={{ borderColor: 'var(--border-color)' }}>
              ...
              <div className="flex items-center gap-2">
                <button onClick={() => { onUseTemplate(previewTemplate.id); setPreviewTemplate(null); }} className="btn-primary text-xs py-2.5 px-5 flex items-center gap-2">{__('tpl.useTemplate')} <ArrowRight size={14} /></button>
